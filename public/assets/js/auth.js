...
function buildNetworkErrorMessage(url) {
  try {
    ...
    if (hostname.endsWith('.app.github.dev')) {
      ...
      return `Cannot reach backend API at ${url}. In workspace, start backend and open app using https://${stableHost}/.`;
    }
  } catch {
  }

  return `Cannot reach backend API at ${url}. Start backend with: cd server && npm run dev`;
}
